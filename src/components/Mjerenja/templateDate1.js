...
const TemplateDate1 = () => {
  ...
  const onUpdateDate = async () => {
    const docRef = doc(db, "StartMeasureDate", `${auth.currentUser.uid}`);
    const docSnap = await getDoc(docRef);

    await setDoc(doc(db, "StartMeasureDate", `${auth.currentUser.uid}`), {
      userDate: date,
    });
      ...
      <Row className="dateRow">
        ...
        {data.userDate && data.userDate !== null ? (
          <p className="filltxtDate">
            <h4 className="dateTxt">Datum:</h4>{" "}
            <h4 className="dateTxtValue"> {data.userDate} </h4>
          </p>
        ) : (
          <p className="filltxtDate">
            <h4 className="dateTxt">Datum:</h4>{" "}
            <h4 className="dateTxtValue"> {displayDate} </h4>
          </p>
        )}
      </Row>
