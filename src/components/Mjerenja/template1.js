...
import TemplateDate1 from "./templateDate1";
import { useAuth } from "../firebase";
import TemplateImage1 from "./templateImg1";
...
const Template1 = (props) => {
  ...
  const onUpdate = async () => {
    //console.log(uid)
    const docRef = doc(db, "StartMeasure", `${auth.currentUser.uid}`);
    const docSnap = await getDoc(docRef);

    /* console.log(docSnap.data()); */

    await setDoc(doc(db, "StartMeasure", `${auth.currentUser.uid}`), {
      userWeight: weight,
      ...
      <Container>
        <Row className="fillRow" style={{ backgroundColor: props.color }}>
          ...
          <Col className="TempSecColumn" xs={5} md={5} lg={5}>
            <TemplateDate1></TemplateDate1>
            <TemplateImage1></TemplateImage1>
            {/* <FontAwesomeIcon
