...
import TemplateDate2 from "./templateDate2";
import { useAuth } from "../firebase";
import TemplateImage2 from "./templateImg2";
...
const Template2 = (props) => {
  ...
  const onUpdate = async () => {
    //console.log(uid)
    const docRef = doc(db, "AfterFirst", `${auth.currentUser.uid}`);
    const docSnap = await getDoc(docRef);

    /* console.log(docSnap.data()); */

    await setDoc(doc(db, "AfterFirst", `${auth.currentUser.uid}`), {
      userWeight: weight,
      ...
      <Container>
        <Row className="fillRow" style={{ backgroundColor: props.color }}>
          ...
          <Col className="TempSecColumn" xs={5} md={5} lg={5}>
            <TemplateDate2></TemplateDate2>
            <TemplateImage2></TemplateImage2>
            {/* <FontAwesomeIcon
