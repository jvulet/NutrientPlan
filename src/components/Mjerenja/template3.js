...
import TemplateDate3 from "./templateDate3";
import { useAuth } from "../firebase";
import TemplateImage3 from "./templateImg3";
...
const Template3 = (props) => {
  ...
  const onUpdate = async () => {
    //console.log(uid)
    const docRef = doc(db, "AfterSecond", `${auth.currentUser.uid}`);
    const docSnap = await getDoc(docRef);

    /* console.log(docSnap.data()); */

    await setDoc(doc(db, "AfterSecond", `${auth.currentUser.uid}`), {
      userWeight: weight,
      ...
      <Container>
        <Row className="fillRow" style={{ backgroundColor: props.color }}>
          ...
          <Col className="TempSecColumn" xs={5} md={5} lg={5}>
            <TemplateDate3></TemplateDate3>
            <TemplateImage3></TemplateImage3>
            {/* <FontAwesomeIcon
